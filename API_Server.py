...
def add_delivery():
    ...
    try:
        ...
        # ---------------- RAW INSERT ----------------
        cursor.execute(
            "INSERT INTO raw_deliveries (order_name, source_location, destination_location, raw_data) VALUES (%s, %s, %s, %s)",
            (order_name, source, destination, json.dumps(data))
        )

        # ---------------- TRANSFORMATION ----------------
        clean_source = int(source)
        clean_destination = int(destination)

        # ---------------- PROCESSED INSERT ----------------
        cursor.execute(
            "INSERT INTO processed_deliveries (order_name, source_location, destination_location, status) VALUES (%s, %s, %s, %s)",
            (order_name, clean_source, clean_destination, "pending")
        )

        # ---------------- FACT INSERT ----------------
        delivery_duration = abs(clean_destination - clean_source) * 10
        is_delayed = delivery_duration > 30

        cursor.execute(
            "INSERT INTO fact_deliveries (agent_id, delivery_duration, is_delayed) VALUES (%s, %s, %s)",
            (1, delivery_duration, is_delayed)
        )
